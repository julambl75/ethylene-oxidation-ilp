...
$$
S(H,\mathcal{T}_{dynamic}^{opt}) = cov_{eg} + \sum_{h \in H}{(50 + 5 \cdot |body(h)|)} + \alpha \cdot \sum_{s \in sensors(H)}{cost(s)}
$$
with $$cov_{eg} = 100 \cdot |uncov_f(H,\mathcal{T}_{dynamic}^{opt})| + 225 \cdot |uncov_n(H,\mathcal{T}_{dynamic}^{opt})|$$
